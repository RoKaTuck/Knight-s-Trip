...
public class TransparentsObject : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        shaders = new Shader[renderers.Length];
        for (int i = 0; i < renderers.Length; i++)
        {
            shaders[i] = renderers[i].material.shader;
        }
    }
    ...
    private IEnumerator BecomeTransparentCoroutine()
    {
        while (true)
        {
            ...
            for (int i = 0; i < renderers.Length; i++)
            {
                ...
                renderers[i].material.shader = Shader.Find("Custom/Smooth_Alpha");
            }
    ...
    private IEnumerator ResetOriginalTransparentCoroutine()
    {
        ...
        while (true)
        {
            ...
            for (int i = 0; i < renderers.Length; i++)
            {
                ...
                color.a += Time.deltaTime;
                renderers[i].material.color = color;
                renderers[i].material.shader = shaders[i];
            }
